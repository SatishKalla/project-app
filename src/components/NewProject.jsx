...
export default function NewProject({ onAdd, onCancel }) {
    ...
    const handleSave = () => {
        const editedTitle = title.current.value;
        const editedDesc = description.current.value;
        const editedDueDate = dueDate.current.value;

        if (editedTitle.trim() === '' || editedDesc.trim() === '' || editedDueDate === '') {
            modal.current.open();
            return;
        }
